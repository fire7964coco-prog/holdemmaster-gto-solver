...
const shortBrowser = () => {
  ...
  if (/SamsungBrowser\/([\d.]+)/.test(ua))
    ...
    return (
      pick(
        ...
        "Samsung Internet ",
        "三星浏览器 "
      , "三星瀏覽器 ", "Samsung Internet ", "Samsung Internet ") + RegExp.$1
    );
  ...
  return pick("기타 브라우저", "Other browser", "その他のブラウザ", "Otro navegador", "Outro navegador", "Anderer Browser", "其他浏览器", "其他瀏覽器", "Autre navigateur", "Browser lain");
};
...
export const errorReportText = () => {
  ...
  const head = [
    ...
    __APP_TARGET__ === "npokers"
      ? pick("npokers 오류 기록", "npokers error log", "npokers エラーログ", "Registro de errores de npokers", "Registro de erros do npokers", "npokers-Fehlerprotokoll", "npokers 错误日志", "npokers 錯誤紀錄", "Journal d'erreurs npokers", "Log error npokers")
      : pick("홀덤마스터 트레이너 오류 기록", "HoldemMaster GTO Trainer error log", "HoldemMaster GTOトレーナー エラーログ", "Registro de errores de HoldemMaster GTO Trainer", "Registro de erros do HoldemMaster GTO Trainer", "Fehlerprotokoll des HoldemMaster GTO Trainers", "HoldemMaster GTO 训练器错误日志", "HoldemMaster GTO 訓練器錯誤紀錄", "Journal d'erreurs du HoldemMaster GTO Trainer", "Log error HoldemMaster GTO Trainer"),
    `${pick("빌드", "Build", "ビルド")} ${__BUILD_ID__} · ${shortBrowser()} · ${pick("화면", "Screen", "画面", "Pantalla", "Tela", "Bildschirm", "屏幕", "螢幕", "Écran", "Layar")} ${window.innerWidth}x${window.innerHeight}`,
    `${pick("설치 실행", "Installed app", "インストール版", "App instalada", "App instalado", "Installierte App", "已安装的应用", "已安裝的應用程式", "App installée", "Aplikasi terinstal")}: ${standalone ? pick("예", "yes", "はい", "sí", "sim", "ja", "是", "是", "oui", "ya") : pick("아니오", "no", "いいえ", "no", "não", "nein", "否", "否", "non", "tidak")}`,
    "",
  ].join("\n");
  const body = records
    .slice()
    .reverse()
    .map((item, index) => {
      const time = new Date(item.t).toLocaleString(pick("ko-KR", "en-US", "ja-JP", "es-MX", "pt-BR", "de-DE", "zh-CN", "zh-TW", "fr-FR", "id-ID"));
...
export const setupErrorCapture = () => {
  ...
  window.addEventListener("error", (event) => {
    ...
    if (event.error) {
      ...
      record(
        `${target.tagName} ${pick("로딩 실패", "failed to load", "読み込み失敗", "no se pudo cargar", "não foi possível carregar", "konnte nicht geladen werden", "加载失败", "載入失敗", "n'a pas pu être chargé", "gagal dimuat")}`,
  ...
  window.addEventListener("unhandledrejection", (event) => {
    ...
    record(
      pick("처리되지 않은 오류: ", "Unhandled rejection: ", "未処理のエラー: ", "Error no controlado: ", "Erro não tratado: ", "Unbehandelter Fehler: ", "未处理的错误：", "未處理的錯誤：", "Erreur non gérée : ", "Error tidak tertangani: ") + String(reason?.message ?? reason),
      String(reason?.stack ?? "")
    );
  });
};
