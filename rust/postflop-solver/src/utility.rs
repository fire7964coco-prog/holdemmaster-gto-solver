...
pub(crate) fn encode_signed_slice(dst: &mut [i16], slice: &[f32]) -> f32 {
    ...
    // `as` cast saturates on overflow and maps NaN to 0, unlike `to_int_unchecked`
    // which traps ("float unrepresentable in integer range") and kills every
    // worker thread when the slice contains inf/NaN (observed on QsJdTs, iter 97)
    dst.iter_mut()
        .zip(slice)
