...
const toUser = (session: Session | null): AccountUser | null => {
  ...
  return {
    ...
    nickname:
      ...
      pick("회원", "Player", "プレイヤー", "jugador", "Jogador", "Spieler", "玩家", "玩家", "Joueur", "Pemain", "Pemain", "खिलाड़ी"),
  };
};
...
export const syncAttempts = async (
  ...
  if (!userId) throw new Error(pick("로그인이 필요합니다", "Please sign in first", "ログインが必要です", "Primero inicia sesión", "Faça login primeiro", "Melde dich zuerst an", "请先登录", "請先登入", "Connecte-toi d'abord", "Silakan login dulu", "Sila log masuk dahulu", "पहले लॉग इन करें"));
