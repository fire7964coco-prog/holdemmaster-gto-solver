...
const toUser = (session: Session | null): AccountUser | null => {
  ...
  return {
    ...
    nickname:
      ...
      pick("회원", "Player", "プレイヤー", "jugador", "Jogador", "Spieler"),
  };
};
...
export const signIn = async (provider: "google" | "kakao") => {
  const supabase = await getClient();
  if (!supabase) throw new Error(pick("계정 기능이 꺼져 있습니다", "Accounts are disabled in this build", "アカウント機能は無効になっています", "Las cuentas están desactivadas en esta versión", "As contas estão desativadas nesta versão", "Konten sind in dieser Version deaktiviert"));
...
export const syncAttempts = async (
  ...
  if (!userId) throw new Error(pick("로그인이 필요합니다", "Please sign in first", "ログインが必要です", "Primero inicia sesión", "Faça login primeiro", "Melde dich zuerst an"));
